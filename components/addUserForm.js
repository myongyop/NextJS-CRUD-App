...
export default function AddUserForm({formData, setFormData}){
  ...
  if(addMutation.isSuccess) return <Success message={"Add Success!"}></Success>
  ...
  return(
    <form className="grid lg:grid-cols-2 w-4/6 gap-4" onSubmit={handleSubmit}>
      ...
      <div className="input-type">
        <input type="text" onChange={setFormData} name="email" className="border w-full px-5 py-3 focus-outline-none rounded-md" placeholder="Emain"></input>
      </div>
      <div className="input-type">
      ...
      <div className="input-type">
        <input type="date" onChange={setFormData} name="date" className="border px-5 py-3 focus:outline-none rounded-md" placeholder="Salary"></input>
      </div>
      ...
      <button className="flex justify-center text-md w-2/6 bg-green-500 text-white px-4 py-2 border rounded-md hover:border-green-500 hover:bg-gray-50 hover:text-green-500">
